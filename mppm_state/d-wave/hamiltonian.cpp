//-*- coding: utf-8; mode: c++ -*-
// Time-stamp: <2017-02-09 15:27:22 shunta>
...
void Hamiltonian::output_gap_angle(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q) {
  ...
  for(nz = 0; nz < 2; nz++) {
    ...
    for(nt = 0; nt <= ntMax; nt++) {
      ...
      Emin = 100.0;
      if(kt <= 0.25 * M_PI) {
        nrMax = long(4000.0 / cos(kt));
      }
      else {
        nrMax = long(4000.0 / sin(kt));
      }

      for(nr = 0; nr <= nrMax; nr++) {
        k[0] = M_PI * (double)nr / 4000.0 * cos(kt);
        k[1] = M_PI * (double)nr / 4000.0 * sin(kt);
