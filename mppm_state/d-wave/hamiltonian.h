//-*- coding: utf-8; mode: c++ -*-
// Time-stamp: <2017-02-03 09:55:51 shunta>
...
namespace Hamiltonian {
  ...
  public:
    ...
    void init_zmatrix(Dvector &k, Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q) {
      long i, j;
      Dvector kplus = k + q * 0.5;
      Dvector kminus = - k + q * 0.5;
    ...
    // パウリ行列
    //
    Complex Sigma(long component, long index) {
      ...
      for (i = 0; i < 4; i++) {
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            mat(i * 4 + (sl / 2) * 2 + (spin / 2), i * 4 + (sl % 2) * 2 + (spin % 2)) = order[i] * sin(k[0] * 0.5) * sin(k[1] * 0.5) * Sigma(0, sl) * imag_unit * Sigma(1, spin);
          }
        }
      }
  ...
  private:
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_intra_layer[layer][sl][spin] =
              ((nnn(k, t) - mu) * Sigma(-1, spin) * Sigma(-1, sl)
               + nn(k, t) * Sigma(-1, spin) * Sigma(0, sl)
               - delta_mu * Sigma(-1, spin) * Sigma(2, sl)) * Sigma(-1, layer);
          }
        }
      }
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_inter_layer1[layer][sl][spin] =
              (real(tnnx(k, t)) * Sigma(-1, spin) * Sigma(-1, sl)
               + real(tnny(k, t)) * Sigma(-1, spin) * Sigma(0, sl)) * Sigma(0, layer)
              - (imag(tnnx(k, t)) * Sigma(-1, spin) * Sigma(-1, sl)
                 + imag(tnny(k, t)) * Sigma(-1, spin) * Sigma(0, sl)) * Sigma(1, layer);
          }
        }
      }
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_inter_layer2[layer][sl][spin] =
              (real(tnny(k, t)) * Sigma(-1, spin) * Sigma(-1, sl)
               + real(tnnx(k, t)) * Sigma(-1, spin) * Sigma(0, sl)) * Sigma(0, layer)
              + (imag(tnny(k, t)) * Sigma(-1, spin) * Sigma(-1, sl)
                 + imag(tnnx(k, t)) * Sigma(-1, spin) * Sigma(0, sl)) * Sigma(1, layer);
          }
        }
      }
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_intra1_ASOC[layer][sl][spin] =
              - alpha[0] * cos(k[0] * 0.5) * cos(k[1] * 0.5) * Sigma(2, spin) * Sigma(1, sl) * Sigma(-1, layer);
          }
        }
      }
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_intra2_ASOC[layer][sl][spin] =
              alpha[1] * ( sin(k[0]) * cos(k[1]) * Sigma(0, spin) - sin(k[1]) * cos(k[0]) * Sigma(1, spin) ) * Sigma(2, sl) * Sigma(2, layer);
          }
        }
      }
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_inter1x_ASOC[layer][sl][spin] =
              alpha[2] * ( cos(k[2] * 0.25) * sin(k[0] * 0.5) * Sigma(0, spin) - 2.0 * sin(k[2] * 0.25) * cos(k[0] * 0.5) * Sigma(2, spin) ) * Sigma(1, sl) * Sigma(1, layer)
              + alpha[2] * ( sin(k[2] * 0.25) * sin(k[0] * 0.5) * Sigma(0, spin) + 2.0 * cos(k[2] * 0.25) * cos(k[0] * 0.5) * Sigma(2, spin) ) * Sigma(1, sl) * Sigma(0, layer);
          }
        }
      }
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_inter1y_ASOC[layer][sl][spin] =
              - alpha[2] * ( cos(k[2] * 0.25) * sin(k[1] * 0.5) * Sigma(1, spin) - 2.0 * sin(k[2] * 0.25) * cos(k[1] * 0.5) * Sigma(2, spin) ) * Sigma(1, sl) * Sigma(1, layer)
              + alpha[2] * ( sin(k[2] * 0.25) * sin(k[1] * 0.5) * Sigma(1, spin) + 2.0 * cos(k[2] * 0.25) * cos(k[1] * 0.5) * Sigma(2, spin) ) * Sigma(1, sl) * Sigma(0, layer);
          }
        }
      }
        ...
        for (sl = 0; sl < 4; sl++) {
          for (spin = 0; spin < 4; spin++) {
            H_inter2_ASOC[layer][sl][spin] =
              alpha[3] * cos(k[2] * 0.5) * ( sin(k[0] * 0.5) * cos(k[1] * 0.5) * Sigma(0, spin) - sin(k[1] * 0.5) * cos(k[0] * 0.5) * Sigma(1, spin) ) * Sigma(2, sl) * Sigma(2, layer);
          }
        }
      }
    ...
    Cmatrix moment_part(double h, Dvector &theta) {
      ...
      for (i = 0; i < 8; i++) {
        for (spin = 0; spin < 4; spin++) {
          mat(i * 2 + (spin / 2), i * 2 + (spin % 2)) = - h * ( cos(theta[i]) * Sigma(0, spin) + sin(theta[i]) * Sigma(1, spin) );
        }
      }
  ...
  // 固有値をファイルに出力する
  void output_eigen_kxky(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q);
  void output_eigen_kykz(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q);
  void output_eigen_kzkx(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q);

  // 固有値をファイルに出力する
  void output_eigen2(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q);

  // バンド基底で表示したギャップの大きさを出力する
  void output_bandgap(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu);

  void test_kx(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q);
  void test_ky(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q);
  void test_kz(Dvector &t, Cvector &order, double h, Dvector &theta, Dvector &alpha, double mu, Dvector &q);
