//-*- coding: utf-8; mode: c++ -*-
// Time-stamp: <2016-11-26 17:17:37 shunta>
...
namespace Hamiltonian {
  ...
  // 各種固定パラメータ
  constexpr long N[3] = {200, 200, 200};             // サイト数
  ...
  // 電子密度を計算する
  void electron_density(Dvector &t, double h, Dvector &theta, Dvector &alpha, double mu);
  ...
  void output_eigen_normal_kx(Dvector &t, double h, Dvector &theta, Dvector &alpha, double mu);

  // Normal part Hamiltonianの代表的な固有値をファイルに出力する
  void output_eigen_normal2(Dvector &t, double h, Dvector &theta, Dvector &alpha, double mu);

  // Normal part energyの非対称部分をファイルに出力する
  void output_asym(Dvector &t, double h, Dvector &theta, Dvector &alpha, double mu);
}
